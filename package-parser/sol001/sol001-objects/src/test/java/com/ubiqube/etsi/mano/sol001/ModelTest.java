...
class ModelTest {
	...
	public ModelTest() {
		reflections = new Reflections("com.ubiqube.parser.tosca.objects", new SubTypesScanner(false));
	}
	...
	private static Object createType(final Class<?> ret) throws NoSuchMethodException, SecurityException, IllegalArgumentException {
		...
		if (ret.isAssignableFrom(Size.class)) {
			return new Size(123L);
		}
		if (ret.isAssignableFrom(Version.class)) {
			return new Version("1.2.3");
		}
		if (ret.isAssignableFrom(Time.class)) {
			return new Time(123L);
		}
		if (ret.isAssignableFrom(Frequency.class)) {
			return new Frequency("1Hz");
		}
		if (ret.isAssignableFrom(Range.class)) {
