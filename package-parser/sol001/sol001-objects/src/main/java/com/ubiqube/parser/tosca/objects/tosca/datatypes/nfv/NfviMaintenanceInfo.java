...
/**
 * Provides information related to the constraints and rules applicable to
 * virtualised resources and their groups impacted due to NFVI maintenance
 * operations
 */
public class NfviMaintenanceInfo extends Root {
	/**
	 * Specifies the allowed migration types in the order of preference in case of
	 * an impact starting with the most preferred type. It is applicable to the
	 * Vdu.Compute node and to the VirtualBlockStorage, VirtualObjectStorage and
	...
	/**
	 * Specifies the minimum notification lead time requested for upcoming impact of
	 * the virtualised resource or their group (i.e. between the notification and
	...
	/**
	 * Specifies the time required by the group to recover from an impact, thus, the
	...
	/**
	 * Specifies for different group sizes the minimum number of instances which
	 * need to be preserved simultaneously within the group of virtualised
	 * resources.
	 */
	@Valid
	...
	/**
	 * Specifies for different group sizes the maximum number of instances that can
	 * be impacted simultaneously within the group of virtualised resources without
	...
	/**
	 * Indicates whether it is requested that at the time of the notification of an
	 * upcoming change that is expected to have an impact on the VNF, virtualised
	 * resource(s) of the same characteristics as the impacted ones is/are provided
	 * to compensate for the impact (TRUE) or not (FALSE).
	 */
	@Valid
	@NotNull
	...
	/**
	 * Specifies the maximum interruption time that can go undetected at the VNF
	 * level and therefore which will not trigger VNF-internal recovery during live
	 * migration. It is applicable to the Vdu.Compute node and to the
	 * VirtualBlockStorage, VirtualObjectStorage and VirtualFileStorage nodes.
	 */
	@Valid
