...
public class NsLcmSubscriptions361Sol005Controller implements NsLcmSubscriptions361Sol005Api {
	...
	public ResponseEntity<LccnSubscription> subscriptionsPost(@Valid final LccnSubscriptionRequest body) {
		return nsLcmSubscriptionsGenericFrontController.create(body, LccnSubscription.class, NsLcmSubscriptions361Sol005Api.class, NsLcmSubscriptions361Sol005Controller::makeLink, NsLcmSubscriptions361Sol005Controller::getSelfLink);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return nsLcmSubscriptionsGenericFrontController.findById(subscriptionId, LccnSubscription.class, NsLcmSubscriptions361Sol005Controller::makeLink);
	}

	@Override
	public ResponseEntity<List<LccnSubscription>> subscriptionsGet(final String filter, final String nextpageOpaqueMarker) {
		return nsLcmSubscriptionsGenericFrontController.search(filter, LccnSubscription.class, NsLcmSubscriptions361Sol005Controller::makeLink);
	}
	...
	private static void makeLink(final LccnSubscription lccnSubscription) {
		...
		self.setHref(linkTo(methodOn(NsLcmSubscriptions361Sol005Api.class).subscriptionsSubscriptionIdGet(lccnSubscription.getId())).withSelfRel().getHref());
		links.setSelf(self);
