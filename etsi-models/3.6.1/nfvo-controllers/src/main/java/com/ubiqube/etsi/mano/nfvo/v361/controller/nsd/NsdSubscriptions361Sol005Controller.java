...
import com.ubiqube.etsi.mano.nfvo.v361.model.nsd.NsdmSubscription;
import com.ubiqube.etsi.mano.nfvo.v361.model.nsd.NsdmSubscriptionLinks;
import com.ubiqube.etsi.mano.nfvo.v361.model.nsd.NsdmSubscriptionRequest;
...
public class NsdSubscriptions361Sol005Controller implements NsdSubscriptions361Sol005Api {
	...
	public ResponseEntity<List<NsdmSubscription>> subscriptionsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return nsdSubscriptionFrontController.search(requestParams, NsdmSubscription.class, NsdSubscriptions361Sol005Controller::makeLink);
	}
	...
	public ResponseEntity<NsdmSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return nsdSubscriptionFrontController.findById(subscriptionId, NsdmSubscription.class, NsdSubscriptions361Sol005Controller::makeLink);
	}
	...
	private static void makeLink(@NotNull final NsdmSubscription subs) {
		...
		self.setHref(linkTo(methodOn(NsdSubscriptions361Sol005Api.class).subscriptionsSubscriptionIdGet(subs.getId())).withSelfRel().getHref());
		nsdmSubscriptionLinks.setSelf(self);
	}

	private static String getSelfLink(@NotNull final NsdmSubscription subs) {
		return linkTo(methodOn(NsdSubscriptions361Sol005Api.class).subscriptionsSubscriptionIdGet(subs.getId())).withSelfRel().getHref();
	}
