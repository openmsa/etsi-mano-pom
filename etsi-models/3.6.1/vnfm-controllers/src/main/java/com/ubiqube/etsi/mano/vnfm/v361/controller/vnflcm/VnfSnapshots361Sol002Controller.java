 ...
 */
@RestController
public class VnfSnapshots361Sol002Controller implements VnfSnapshots361Sol002Api {
	private final VnfSnapshotsFrontController vnfSnapshotsFrontController;

	public VnfSnapshots361Sol002Controller(final VnfSnapshotsFrontController vnfSnapshotsFrontController) {
		super();
	...
	public ResponseEntity<List<VnfSnapshotInfo>> vnfSnapshotsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return vnfSnapshotsFrontController.search(requestParams, nextpageOpaqueMarker, VnfSnapshotInfo.class, VnfSnapshots361Sol002Controller::makeLinks);
	}

	@Override
	public ResponseEntity<VnfSnapshotInfo> vnfSnapshotsPost(@Valid final CreateVnfSnapshotInfoRequest body) {
		return vnfSnapshotsFrontController.create(body, VnfSnapshotInfo.class, VnfSnapshots361Sol002Controller::makeLinks);
	}
	...
	public ResponseEntity<VnfSnapshotInfo> vnfSnapshotsVnfSnapshotInfoIdGet(final String vnfSnapshotInfoId) {
		return vnfSnapshotsFrontController.findById(vnfSnapshotInfoId, VnfSnapshotInfo.class, VnfSnapshots361Sol002Controller::makeLinks);
	}
	...
	private static void makeLinks(final VnfSnapshotInfo subscription) {
		...
		link.setHref(linkTo(methodOn(VnfSnapshots361Sol002Api.class).vnfSnapshotsVnfSnapshotInfoIdGet(subscription.getId())).withSelfRel().getHref());
		links.setSelf(link);
