...
@RequestMapping(value = "/sol002/vnflcm/v2", headers = { "Version=2.2.0" })
@RolesAllowed({ "ROLE_EM" })
public interface VnfLcmSubscriptions361Sol002Api {

	Logger log = LoggerFactory.getLogger(VnfLcmSubscriptions361Sol002Api.class);
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "200 OK The list of subscriptions has been queried successfully. The response body shall contain in an array the representations of all active subscriptions of the functional block that invokes the method, i.e. zero or more representations of lifecycle change notification subscriptions as defined in clause 5.5.2.16. ", content = @Content(schema = @Schema(implementation = LccnSubscription.class))),
	...
	default ResponseEntity<LccnSubscription> subscriptionsGet(
			...
			@Parameter(in = ParameterIn.QUERY, description = "Attribute-based filtering expression according to clause 5.2 of ETSI GS NFV-SOL 013 [6]. The VNFM shall support receiving this parameter as part of the URI query string. The EM may supply this parameter. All attribute names that appear in the LccnSubscription and in data types referenced from it shall be supported by the VNFM in the filter expression. ", schema = @Schema()) @Valid @RequestParam(value = "filter", required = false) final String filter,
	...
	default ResponseEntity<LccnSubscription> subscriptionsPost(
			@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version,
			@Parameter(in = ParameterIn.DEFAULT, description = "Details of the subscription to be created.", required = true, schema = @Schema()) @Valid @RequestBody final LccnSubscriptionRequest body,
			@Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
			if (getAcceptHeader().get().contains("application/json")) {
				try {
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "200 OK The operation has completed successfully. The response body shall contain a representation of the \"Individual subscription\" resource. ", content = @Content(schema = @Schema(implementation = LccnSubscription.class))),
