...
import com.ubiqube.etsi.mano.em.v361.model.vnfpm.ThresholdLinks;
import com.ubiqube.etsi.mano.em.v361.model.vnfpm.ThresholdModifications;
...
public class Thresholds361Sol003Controller implements Thresholds361Sol003Api {
	...
	public Thresholds361Sol003Controller(final VnfmThresholdFrontController vnfmThresholdFrontController) {
	...
	public ResponseEntity<Threshold> thresholdsPost(@Valid final CreateThresholdRequest createThresholdRequest) {
		return vnfmThresholdFrontController.thresholdsCreate(createThresholdRequest, Threshold.class, Thresholds361Sol003Controller::makeLinks, Thresholds361Sol003Controller::getSelfLink);
	}
	...
	public ResponseEntity<Threshold> thresholdsThresholdIdGet(final String thresholdId) {
		return vnfmThresholdFrontController.findById(thresholdId, Threshold.class, Thresholds361Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<String> thresholdsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return vnfmThresholdFrontController.search(requestParams, nextpageOpaqueMarker, Threshold.class, Thresholds361Sol003Controller::makeLinks);
	}
	...
	private static void makeLinks(final Threshold x) {
		...
		Link link = new Link();
		link.setHref(linkTo(methodOn(Thresholds361Sol003Api.class).thresholdsThresholdIdGet(x.getId())).withSelfRel().getHref());
		links.setSelf(link);
	...
	private static String getSelfLink(final Threshold threshold) {
		return linkTo(methodOn(Thresholds361Sol003Api.class).thresholdsThresholdIdGet(threshold.getId().toString())).withSelfRel().getHref();
	}
