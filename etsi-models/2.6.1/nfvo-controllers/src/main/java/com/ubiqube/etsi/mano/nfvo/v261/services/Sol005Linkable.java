...
public class Sol005Linkable implements Linkable {
	...
	public VnfPkgInfoLinks getVnfLinks(final String vnfPkgId) {
		...
		vnfd.setHref(linkTo(methodOn(VnfPackage261Sol005Api.class).vnfPackagesVnfPkgIdVnfdGet(vnfPkgId, null)).withSelfRel().getHref());
		links.setVnfd(vnfd);

		final Link packageContent = new Link();
		packageContent.setHref(linkTo(methodOn(VnfPackage261Sol005Api.class).vnfPackagesVnfPkgIdPackageContentGet(vnfPkgId, "")).withSelfRel().getHref());
	...
	public PkgmLinks createVnfPackageOnboardingNotificationLinks(final UUID vnfPkgId, final String vnfdId, final UUID subscriptionId) {
		...
		final Link subscription = createVnfPackagesVnfPkgInfoLinksSelf(
				linkTo(methodOn(VnfSubscription261Sol005Api.class).subscriptionsSubscriptionIdGet(subscriptionId.toString())).withSelfRel().getHref());
		ret.setSubscription(subscription);

		final Link vnfPackage = createVnfPackagesVnfPkgInfoLinksSelf(
				linkTo(methodOn(VnfPackage261Sol005Api.class).vnfPackagesVnfPkgIdGet(vnfPkgId.toString(), "")).withSelfRel().getHref());
		ret.setVnfPackage(vnfPackage);

		final Link vnfVnfdId = createVnfPackagesVnfPkgInfoLinksSelf(
				linkTo(methodOn(VnfPackage261Sol005Api.class).vnfPackagesVnfPkgIdVnfdGet(vnfdId, null)).withSelfRel().getHref());
		ret.setVnfPackageByVnfdId(vnfVnfdId);
		return ret;
	}
	
	@Override
	public VnfIndicatorValueChangeNotificationLinks createVnfIndicatorValueChangeNotificationLinks(final String vnfIndicator, final String vnfInstanceId, final UUID subscriptionId) {
	...
	public String getSelfLink(final VnfPkgInfo vnfPkgInfo) {
		return linkTo(methodOn(VnfPackage261Sol005Api.class).vnfPackagesVnfPkgIdGet(vnfPkgInfo.getId(), null)).withSelfRel().getHref();
	}
}
