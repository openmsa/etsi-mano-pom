 ...
 * the VIM. * NOTE 2: It is possible to have several ExtManagedVirtualLinkData
 * for the same VNF internal VL in case of a multi-site VNF spanning several
 * VIMs. The set of ExtManagedVirtualLinkData corresponding to the same VNF
 * internal VL shall indicate so by referencing to the same VnfVirtualLinkDesc
 * and externally-managed multi-site VL instance (refer to clause 4.4.1.12). *
...
public class ChangeVnfFlavourRequest {
	 ...
	 * \"VnfInstance\" structure (see clause 5.5.2.2) and the related overriding
	 * information provided in the \"Grant\" structure (see clause 9.5.2.3): Even if
	 * the VNF is not in fully scaled-out state after changing the flavour, the API
	 * consumer shall provide enough CP configuration records to allow connecting
	 * the VNF instance, fully scaled out in all scaling aspects, to the external
	 * VLs.
	 *
	...
	public ChangeVnfFlavourRequest vimConnectionInfo(final Map<String, VimConnectionInfo> vimConnectionInfo) {
		this.vimConnectionInfo = vimConnectionInfo;
		return this;
	}

	public ChangeVnfFlavourRequest putVimConnectionInfoItem(final String key, final VimConnectionInfo vimConnectionInfoItem) {
	...
	@Override
	public int hashCode() {
		return Objects.hash(newFlavourId, instantiationLevelId, targetScaleLevelInfo, extVirtualLinks, extManagedVirtualLinks, vimConnectionInfo, additionalParams, extensions, vnfConfigurableProperties);
	}
