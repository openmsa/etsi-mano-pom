...
/**
 * This type provides additional service information of the virtual CP instance
 * used to expose properties of the virtual CP to NFV-MANO. NOTE: This attribute
 * shall only be present if additional information is needed to identify the
 * service termination within the VNF, such as for example a URL path
 * information in an HTTP request required to allow a single virtual CP IP
 * address to be used for several HTTP based services that use the same port
 * number.
 */
@Schema(description = "This type provides additional service information of the virtual CP instance used to expose properties of the  virtual CP to NFV-MANO. NOTE: This attribute shall only be present if additional information is needed to identify the service        termination within the VNF, such as for example a URL path information in an HTTP request required        to allow a single virtual CP IP address to be used for several HTTP based services that use the        same port number. ")
@Validated
