 ...
 * operation. NOTE: If the VNF is still in service, requesting forceful
 * termination can adversely impact network service.
 */
@Schema(description = "This type represents request parameters for the \"Terminate VNF\" operation. NOTE: If the VNF is still in service, requesting forceful termination can        adversely impact network service. ")
@Validated

public class TerminateVnfRequest {
	/**
	 * Indicates whether forceful or graceful termination is requested. See note.
	 * Permitted values: - FORCEFUL: The VNFM will shut down the VNF and release the
	 * resources immediately after accepting the request. - GRACEFUL: The VNFM will
	 * first arrange to take the VNF out of service after accepting the request.
	 * Once the operation of taking the VNF out of service finishes (irrespective of
	 * whether it has succeeded or failed) or once the timer value specified in the
