 ...
 * (refer to clause 6.5.3.27). NOTE 3: The target size for VNF instantiation may
 * be specified in either instantiationLevelId or targetScaleLevelInfo, but not
 * both. If none of the two attributes (instantiationLevelId or
 * targetScaleLevelInfo) are present, the default instantiation level as
 * declared in the VNFD shall be used. NOTE 4: If targetScaleLevelInfo is
 * specified, information provided in targetScaleLevelInfo shall be used for
 * instantiating scalable constituents of the VNF (e.g, VDUs/VLs). For scaling
 * aspects not specified in targetScaleLevelInfo or for the VNF constituents
 * (e.g., VDUs/VLs) that are not scalable, the default instantiation level as
 * declared in the VNFD shall be used for instantiation.
 */
...
public class ChangeVnfFlavourData implements OneOfChangeVnfFlavourData {
	...
	@Override
	public int hashCode() {
		return Objects.hash(vnfInstanceId, newFlavourId, instantiationLevelId, targetScaleLevelInfo, extVirtualLinks, extManagedVirtualLinks, additionalParams, extensions, vnfConfigurableProperties);
	}
