...
/**
 * This type represents parameters to connect to a VIM, a CISM, a CIR or a MCIOP
 * repository for managing the resources of a VNF instance. This structure is
 * used to convey VIM-related, CISM-related, CIR-related, or
 * MCIOP-repository-related parameters over the Or-Vnfm interface. Additional
 * parameters for a VIM, a CISM, a CIR or a MCIOP repository may be configured
 * into the VNFM by means outside the scope of the present document and bound to
 * the identifier of that VIM. * NOTE 1: If applicable, this attribute also
 ...
 * connectivity between VNFM and VIM, CISM, CIR or MCIOP repository is provided
 * through a secure connection over HTTP Secure (HTTP over SSL/TLS), and the
 * connection might also be established through a VPN (for example TLS-based VPN
 * tunnelling) for site-to-site connection, the \&quot;accessInfo\&quot; JSON
 * data structure, and the sensitive data related information
...
public class VimConnectionInfo {
	 ...
	 * \"accessInfo\" attributes, based on the type of the VIM., CISM, CIR or MCIOP
	 * repository. The set of permitted values is expected to change over time as
	 * new types or versions of VIMs become available. The ETSI NFV registry of
	 * VIM-related information [i.3] provides access to information about
	 * VimConnectionInfo definitions for various VIM, CISM, CIR or MCIOP repository
	 * types. The structure of the registry is defined in annex C.
	 *
	 * @return vimType
	 **/
	@Schema(required = true, description = "Discriminator for the different types of the VIM information. The value of this attribute determines the structure of the \"interfaceInfo\" and \"accessInfo\" attributes, based on the type of the VIM., CISM, CIR or MCIOP repository. The set of permitted values is expected to change over time as new types or versions of VIMs become available. The ETSI NFV registry of VIM-related information [i.3] provides access to information about VimConnectionInfo definitions for various VIM, CISM, CIR or MCIOP repository types. The structure of the registry is defined in annex C. ")
	@NotNull
