...
@RestController
public class AlarmSubscriptions431Sol002Controller implements AlarmSubscriptions431Sol002Api {
	private final FaultMngtSubscriptionsFrontController faultMngtSubscriptionsFrontController;

	public AlarmSubscriptions431Sol002Controller(final FaultMngtSubscriptionsFrontController faultMngtSubscriptionsFrontController) {
	...
	public ResponseEntity<FmSubscription> subscriptionsPost(@Valid final FmSubscriptionRequest fmSubscriptionRequest) {
		return faultMngtSubscriptionsFrontController.create(fmSubscriptionRequest, FmSubscription.class, AlarmSubscriptions431Sol002Api.class, AlarmSubscriptions431Sol002Controller::makeLinks, AlarmSubscriptions431Sol002Controller::makeSelf);
	}
	...
	private static void makeLinks(final FmSubscription subscription) {
		...
		link.setHref(linkTo(methodOn(AlarmSubscriptions431Sol002Api.class).subscriptionsSubscriptionIdGet(subscription.getId())).withSelfRel().getHref());
		links.setSelf(link);
		subscription.setLinks(links);
	}

	private static String makeSelf(final FmSubscription subscription) {
		return linkTo(methodOn(AlarmSubscriptions431Sol002Api.class).subscriptionsSubscriptionIdGet(subscription.getId())).withSelfRel().getHref();
	}
