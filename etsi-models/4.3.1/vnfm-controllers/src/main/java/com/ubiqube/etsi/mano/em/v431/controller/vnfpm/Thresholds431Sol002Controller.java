...
@RestController
public class Thresholds431Sol002Controller implements Thresholds431Sol002Api {

	private final VnfmThresholdFrontController vnfmThresholdFrontController;

	public Thresholds431Sol002Controller(final VnfmThresholdFrontController vnfmThresholdFrontController) {
	...
	public ResponseEntity<Threshold> thresholdsPost(@Valid final CreateThresholdRequest createThresholdRequest) {
		return vnfmThresholdFrontController.thresholdsCreate(createThresholdRequest, Threshold.class, Thresholds431Sol002Controller::makeLinks, Thresholds431Sol002Controller::getSelfLink);
	}
	...
	public ResponseEntity<Threshold> thresholdsThresholdIdGet(final String thresholdId) {
		return vnfmThresholdFrontController.findById(thresholdId, Threshold.class, Thresholds431Sol002Controller::makeLinks);
	}
	...
	public ResponseEntity<String> thresholdsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return vnfmThresholdFrontController.search(requestParams, nextpageOpaqueMarker, Threshold.class, Thresholds431Sol002Controller::makeLinks);
	}
	...
	private static void makeLinks(final Threshold x) {
		...
		Link link = new Link();
		link.setHref(linkTo(methodOn(Thresholds431Sol002Api.class).thresholdsThresholdIdGet(x.getId())).withSelfRel().getHref());
		links.setSelf(link);
	...
	private static String getSelfLink(final Threshold threshold) {
		return linkTo(methodOn(Thresholds431Sol002Api.class).thresholdsThresholdIdGet(threshold.getId().toString())).withSelfRel().getHref();
	}
