 ...
 */
package com.ubiqube.etsi.mano.vnfm.v431.controller.vnfsnapshotpkgm;
...
import com.ubiqube.etsi.mano.vnfm.v431.model.vnfsnapshotpkgm.VnfSnapshotPkgInfo;
import com.ubiqube.etsi.mano.vnfm.v431.model.vnfsnapshotpkgm.VnfSnapshotPkgInfoLinks;
import com.ubiqube.etsi.mano.vnfm.v431.model.vrqan.Link;

@RestController
public class VnfSnapshotPackages431Sol003Controller implements VnfSnapshotPackages431Sol003Api {

	private final VnfSnapshotsFrontController vnfSnapshotsFrontController;

	public VnfSnapshotPackages431Sol003Controller(final VnfSnapshotsFrontController vnfSnapshotsFrontController) {
	...
	public ResponseEntity<List<VnfSnapshotPkgInfo>> vnfSnapshotPackagesGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return vnfSnapshotsFrontController.search(requestParams, nextpageOpaqueMarker, VnfSnapshotPkgInfo.class, VnfSnapshotPackages431Sol003Controller::makeLinks);
	}

	@Override
	public ResponseEntity<VnfSnapshotPkgInfo> vnfSnapshotPackagesVnfSnapshotPkgIdGet(final String vnfSnapshotInfoId) {
		return vnfSnapshotsFrontController.findById(vnfSnapshotInfoId, VnfSnapshotPkgInfo.class, VnfSnapshotPackages431Sol003Controller::makeLinks);
	}
	...
	private static void makeLinks(final VnfSnapshotPkgInfo subscription) {
		...
		link.setHref(linkTo(methodOn(VnfSnapshotPackages431Sol003Api.class).vnfSnapshotPackagesVnfSnapshotPkgIdGet(subscription.getId())).withSelfRel().getHref());
		links.setSelf(link);
