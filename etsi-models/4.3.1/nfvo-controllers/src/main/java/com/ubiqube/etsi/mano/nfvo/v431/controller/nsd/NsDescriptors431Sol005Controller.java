...
public class NsDescriptors431Sol005Controller implements NsDescriptors431Sol005Api {
	...
	public ResponseEntity<String> nsDescriptorsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return nsDescriptorGenericFrontController.search(requestParams, NsdInfo.class, NsDescriptors431Sol005Controller::makeLinks);
	}
	...
	private static void makeLinks(@Nonnull final NsdInfo nsdInfo) {
		...
		final String _nsdContent = linkTo(methodOn(NsDescriptors431Sol005Api.class).nsDescriptorsNsdInfoIdNsdContentGet(id, "")).withSelfRel().getHref();
	...
	private static String makeSelfLink(final NsdInfo nsdInfo) {
		return linkTo(methodOn(NsDescriptors431Sol005Api.class).nsDescriptorsNsdInfoIdGet(nsdInfo.getId())).withSelfRel().getHref();
	}
