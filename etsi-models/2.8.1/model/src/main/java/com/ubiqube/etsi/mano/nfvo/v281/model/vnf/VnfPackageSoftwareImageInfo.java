...
public class VnfPackageSoftwareImageInfo {
   ...
   * to access a copy of the image artifact. For a software image contained as a
   * file in the VNF package, this attribute shall be present, and the value of
   * this attribute shall start with the name of the first segment in the path in
   * the package, i.e., it shall not be prefixed by path separator characters such
   * as \".\" and \"/\". EXAMPLE: foo/bar/m%40ster.vhd For an external software
   * image represented as a URI in the VNF descriptor, this attribute shall be
   * present if the image artifact has been downloaded by the NFVO and shall be
   * absent otherwise. If present, it shall contain the artifactPath under which
   * the image artifact can be obtained using the \"Individual artifact in a VNF
  ...
  /**
   * URI of the image artifact as defined in the VNF package manifest. Shall be
   * present if the image artifact is external to the VNF package and shall be
   * absent otherwise. EXAMPLE: https://example.com/m%40ster.vhd
   *
   * @return imageUri
   **/
  @Schema(description = "URI of the image artifact as defined in the VNF package manifest.  Shall be present if the image artifact is external to the VNF  package and shall be absent otherwise. EXAMPLE: https://example.com/m%40ster.vhd ")
