 ...
 *
 */
@Api(value = "change_state_ops", description = "the change_state_ops API")
public interface ChangeStateOpsApi {

	@ApiOperation(value = "", nickname = "changeStateOpsChangeStateOpOccIdGet", notes = "The API consumer can use the GET method to retrieve status  information about a change state operation occurrence by reading  an \"Individual change state operation occurrence\" resource. This method shall follow the provisions specified in the tables  5.5.9.3.2-1 and 5.5.9.3.2-2 for URI query parameters, request  and response data structures, and response codes. ", response = ChangeStateOpOcc.class, tags = {})
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "200 OK Shall be returned when information about an \"Individual change  state operation occurrence\" resource has been read successfully. The response body shall contain status information about a change  state operation occurrence, as defined in clause 5.6.2.9. ", response = ChangeStateOpOcc.class),
	...
	@ApiResponses(value = {
			...
			@ApiResponse(code = 503, message = "503 SERVICE UNAVAILABLE If the API producer encounters an internal overload situation of itself or of a system it relies on, it should respond with this response code, following the provisions in IETF RFC 7231 for the use of the \"Retry-After\" HTTP header and for the alternative to refuse the connection. The \"ProblemDetails\" structure may be omitted. ", response = ProblemDetails.class),
			@ApiResponse(code = 504, message = "504 GATEWAY TIMEOUT If the API producer encounters a timeout while waiting for a response from an upstream server (i.e. a server that the API producer communicates with when fulfilling a request), it should respond with this response code. ", response = ProblemDetails.class) })
	@RequestMapping(value = "/change_state_ops", produces = { "application/json" }, method = RequestMethod.GET)
	ResponseEntity<List<ChangeStateOpOcc>> changeStateOpsGet(@ApiParam(value = "Marker to obtain the next page of a paged response. Shall be supported by  the NFV-MANO functional entity if the entity supports alternative 2 (paging)  according to clause 5.4.2.1 of ETSI GS NFV-SOL 013 for this resource. ") @Valid @RequestParam(value = "nextpage_opaque_marker", required = false) String nextpageOpaqueMarker);
