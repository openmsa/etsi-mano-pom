 ...
 */
@RestController
public class NsSubscriptions331Sol005Controller implements NsSubscriptions331Sol005Api {
	private final NsdSubscriptionFrontController nsdSubscriptionFrontController;

	public NsSubscriptions331Sol005Controller(final NsdSubscriptionFrontController nsdSubscriptionFrontController) {
	...
	public ResponseEntity<List<NsdmSubscription>> subscriptionsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return nsdSubscriptionFrontController.search(requestParams, NsdmSubscription.class, NsSubscriptions331Sol005Controller::makeLink);
	}
	...
	public ResponseEntity<NsdmSubscription> subscriptionsPost(final NsdmSubscriptionRequest body) {
		return nsdSubscriptionFrontController.create(body, NsdmSubscription.class, NsSubscriptions331Sol005Api.class, NsSubscriptions331Sol005Controller::makeLink, NsSubscriptions331Sol005Controller::getSelfLink);
	}
	...
	public ResponseEntity<NsdmSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return nsdSubscriptionFrontController.findById(subscriptionId, NsdmSubscription.class, NsSubscriptions331Sol005Controller::makeLink);
	}
	...
	private static void makeLink(@NotNull final NsdmSubscription subs) {
		...
		self.setHref(linkTo(methodOn(NsSubscriptions331Sol005Api.class).subscriptionsSubscriptionIdGet(subs.getId())).withSelfRel().getHref());
		nsdmSubscriptionLinks.setSelf(self);
	}

	private static String getSelfLink(@NotNull final NsdmSubscription subs) {
		return linkTo(methodOn(NsSubscriptions331Sol005Api.class).subscriptionsSubscriptionIdGet(subs.getId())).withSelfRel().getHref();
	}
}
