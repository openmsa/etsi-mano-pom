...
import com.ubiqube.etsi.mano.em.v331.model.vnffm.AlarmLinks;
...
public class Alarms331Sol002Controller implements Alarms331Sol002Api {
	...
	public Alarms331Sol002Controller(final AlarmFrontController alarmFrontController) {
		super();
	...
	public ResponseEntity<Alarm> alarmsAlarmIdGet(final String alarmId) {
		return alarmFrontController.findById(alarmId, Alarm.class, Alarms331Sol002Controller::makeLinks);
	}
	...
	public ResponseEntity<String> alarmsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return alarmFrontController.search(requestParams, Alarm.class, Alarms331Sol002Controller::makeLinks);
	}

	private static void makeLinks(final Alarm alarm) {
		final AlarmLinks links = new AlarmLinks();
		Link link = new Link();
		link.setHref(linkTo(methodOn(Alarms331Sol002Api.class).alarmsAlarmIdGet(alarm.getId())).withSelfRel().getHref());
		links.setSelf(link);
