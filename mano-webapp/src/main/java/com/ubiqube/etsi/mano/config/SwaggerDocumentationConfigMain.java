...
public class SwaggerDocumentationConfigMain {
	...
	OpenAPI openApiMain() {
		return new OpenAPI()
				.components(new Components().addSecuritySchemes("basicScheme", new SecurityScheme().type(SecurityScheme.Type.HTTP).scheme("basic"))
						...
						.addHeaders("Version", new Header().description("Vesion header").schema(new StringSchema()._default("application/json"))))
	...
	GroupedOpenApi customImplementationMain() {
		return GroupedOpenApi.builder()
				.group("main")
				.packagesToScan("com.ubiqube.etsi.mano.controllers")
				.build();
	}
