...
public class DnsContributorV3 extends AbstractVnfmContributorV3<Object> {
	...
	public List<SclableResources<Object>> contribute(final VnfPackage bundle, final VnfBlueprint parameters) {
		...
		vnfPackage.getVnfVl().forEach(x -> {
			...
			ret.add(create(DnsZone.class, x.getToscaName(), 1, dnsZoneTask, parameters.getInstance()));
		});
		vnfPackage.getVnfLinkPort().forEach(x -> {
			...
			dht.setToscaName(x.getVirtualLink() + "-" + x.getToscaName());
			dht.setHostname(x.getVirtualBinding());
			dht.setNetworkName(x.getVirtualLink());
			dht.setZoneId(x.getToscaName() + ".mano.vm");
			dht.setType(ResourceTypeEnum.DNSHOST);
			ret.add(create(DnsHost.class, dht.getToscaName(), 1, dht, parameters.getInstance()));
		});
		return ret;
	}
